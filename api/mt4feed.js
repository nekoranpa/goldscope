let cache = {
  bid:0,ask:0,mid:0,spread:0,symbol:'GOLD',
  ts:0,ohlcv:[],updatedAt:0,
};
const SECRET_KEY = process.env.MT4_SECRET_KEY || 'goldscope2026';
...
export default async function handler(req, res) {
  ...
  if(req.method==='POST'){
    try{
      ...
      if(!mid || isNaN(mid) || mid < 100 || mid > 100000){
        return res.status(400).json({error:'Invalid price: '+body.mid});
      }

      const bars=[];
      if(body.ohlcv && typeof body.ohlcv === 'string'){
      ...
      cache={
        ...
        mid:mid,
        spread:parseFloat(body.spread)||0,
        symbol:body.symbol||'GOLD',
      ...
      console.error('[mt4feed POST error]', e);
